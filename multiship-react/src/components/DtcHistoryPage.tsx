...
import { apiBatchPath, apiBatchesPath, bulkBatchPath, bulkPaths, settingsPaths, dtcBatchPath } from '../routes/workspaceRoutes'
...
// D2C History — copy of DataHistoryPage.tsx wired to dtcService (Oracle NDS view).
// Same UI shape, DTC data source. The `apiBatches` prop is forced true so this
// renders the single-list "API Batches" shape (no imports/labels/trash tabs).
export default function DtcHistoryPage({ apiBatches = true }: { apiBatches?: boolean } = {}) {
  ...
  useEffect(() => {
    if (!apiBatches && !batchPageSlug && tab && !BULK_TABS.some((t) => t.key === tab)) navigate(bulkPaths.imports, { replace: true })
  ...
  useEffect(() => {
    ...
    if (isApi !== apiBatches) navigate(isApi ? apiBatchPath(b.slug) : bulkBatchPath(b.slug), { replace: true })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const rowsOfBatch = async (b: ImportBatchSummary): Promise<OrderImportRow[]> => {
    return (await orderImportService.historyRows(b.slug, { view: 'live', size: 5000 })).data?.rows ?? []
  }
   ...
   */
  const ensureRows = (id: number, rowsLoaded = false, hint?: ImportBatchSummary) => {
    // hint: the just-fetched batch, so this can run before setBatches has landed.
  ...
  /** D2C History — always the DTC (Oracle NDS) view; Trash preserved. */
  const listView: BulkView = viewTrash ? 'TRASH' : 'DTC'
  ...
  const fetchBatches = async (fresh = true): Promise<{ data: ImportBatchSummary[]; rows?: OrderImportRow[]; page?: { total: number; pages: number }; summary?: BulkSummary | null; summaryView?: string }> => {
    // The batch page's "list" is that one batch (live or in Trash, any source): its header only —
    // its rows come a page at a time.
    if (batchPageSlug != null) {
      const res = await bulkService.batch(batchPageSlug)
  ...
  /** D2C: pull the Oracle NDS view's pending rows in as one new batch (or reopen the same one). */
  const fetchFromWms = async () => {
    setFetchingWms(true)
    try {
      const r = (await dtcService.pull()).data
      if (r && !r.configured) {
        notify.info('The NDS link is not set up on the server (multiship.oracle.enabled=false).')
      } else if (r) {
        if (r.imported > 0) {
          notify.success(`NDS: ${r.imported} shipment(s) imported as a new batch${r.failed ? ` · ${r.failed} skipped` : ''}.`)
        } else if (r.importBatchId != null) {
          notify.info('These shipments were already fetched — opening the existing batch.')
        } else {
          notify.info('The NDS view has no pending shipments to fetch.')
        }
        if (r.importBatchSlug) navigate(dtcBatchPath(r.importBatchSlug))
      }
    } catch (e) {
      notify.apiError(e, 'Could not reach the NDS view.')
    } finally {
  ...
  const load = async (fresh = true) => {
    ...
    setRefreshing(true)
    try {
      const fetched = await fetchBatches(fresh)
      if (!applyFetch(seq, view, fetched)) return
      // The batch page's id comes from the batches state, a render behind: use the fetched batch.
      if (batchPageSlug != null) {
        const target = fetched.data.find((b) => b.slug === batchPageSlug)
        if (target) ensureRows(target.id, true, target)   // the rows come a page at a time (below)
      }
  ...
  const generate = async (id: number, isRetry: boolean, allowDuplicate = false) => {
    ...
    try {
      ...
      if (updated && (updated.status || '').toUpperCase() === 'IN_PROGRESS') {
        ...
        try {
          detail = (await bulkService.batch(slug)).data
        } catch {
  ...
  useEffect(() => {
    if (batchPageId == null || batchPageSlug == null) return
    let gone = false
    orderImportService.historyRows(batchPageSlug, { view: rowsView, q: gridQ || undefined, page: gridPage, size: gridPageSize })
      .then((res) => {
        const pg = res.data
        if (gone || !pg) return
    ...
    return () => { gone = true }
  }, [batchPageId, batchPageSlug, rowsView, gridQ, gridPage, gridPageSize, rowsTick])
  ...
  const exportBatchRows = async (b: ImportBatchSummary) => {
    try {
      const pg = (await orderImportService.historyRows(b.slug, { view: rowsView, q: gridQ || undefined, size: 5000 })).data
      if (!pg) return
  ...
  const openFix = (batchId: number, rowNumber: number) => {
    ...
    const slug = batches.find((b) => b.id === batchId)?.slug
    if (!slug) return
    orderImportService.historyRows(slug, { rowNumber })
  ...
  const recheckFix = async (batchId: number, rowNumber: number) => {
    ...
    if (!slug) return
    try {
      const r = (await orderImportService.historyRows(slug, { rowNumber })).data?.rows?.[0]
      if (!r) return
      setFixRowOffPage(r)
    ...
    <>
      ...
      {canPullWms && isApiTab ? (
        ...
        >
          ...
          {fetchingWms ? 'Fetching…' : 'Fetch from NDS'}
        </button>
      ) : null}
    ...
    <div className="space-y-3 pb-8">
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2 px-1 pt-1">
        ...
        >
          ...
          {apiBatches ? 'D2C History' : 'Bulk Mailer'}
        </h2>
      ...
      >
        {(
          <AdvancedDataTable<ImportBatchSummary>
            ...
            onRowClick={(b) => navigate(isApiTab ? apiBatchPath(b.slug) : bulkBatchPath(b.slug))}
              ...
              (summary?.total ?? 0) === 0 && !filters.anyFilterActive ? (
                <p className="px-5 py-10 text-center text-sm text-[#6b5c42]">
                  {viewTrash
                    ...
                    : isApiTab
                      ? (canPullWms ? 'No D2C batches yet. Use Fetch from NDS to pull the pending shipments in.' : 'No D2C batches yet. An admin can use Fetch from NDS to pull the pending shipments in.')
