...
function EditDrawer({
  ...
  // V90 dispatch gates — default TRUE for new rows to match V90 column default
  // (an admin who leaves writeback on wants it to fire everywhere by default).
...
function WritebackTab({
  ...
  // V90 — filter WHEN writeback fires, by shipment source + channel.
  const sourceRows: Array<{ key: string; label: string; desc: string; checked: boolean; onChange: (v: boolean) => void }> = [
    { key: 'src-manual', label: 'Manual (/orders/new)',
      desc: 'Labels generated from the manual shipment form.',
      checked: wbSrcManual, onChange: setWbSrcManual },
    { key: 'src-bulk', label: 'Bulk (CSV import + queue)',
      desc: 'Labels generated by bulk import or the BulkLabel queue.',
      checked: wbSrcBulk, onChange: setWbSrcBulk },
  ...
  const channelRows: Array<{ key: string; label: string; desc: string; checked: boolean; onChange: (v: boolean) => void }> = [
    { key: 'ch-d2c', label: 'D2C — Direct to consumer',
      desc: 'Orders whose channel resolves to D2C.',
      checked: wbChanD2c, onChange: setWbChanD2c },
    { key: 'ch-b2b', label: 'B2B — Business to business',
      desc: 'Orders whose channel resolves to B2B.',
      checked: wbChanB2b, onChange: setWbChanB2b },
  ]
  ...
  return (
    <div className="space-y-3">
      ...
      <FlagList idPrefix="wb" rows={rows} />

      <div className="pt-3">
        <p className="mb-1 text-[12.5px] font-semibold text-slate-800">Fires on shipment source</p>
        <p className="mb-2 text-[11.5px] text-slate-500">
          Uncheck a source to skip writeback for shipments coming from that surface.
        </p>
        <FlagList idPrefix="wb" rows={sourceRows} />
      </div>

      <div className="pt-3">
        <p className="mb-1 text-[12.5px] font-semibold text-slate-800">Fires on order channel</p>
        <p className="mb-2 text-[11.5px] text-slate-500">
          Uncheck a channel to skip writeback for orders classified as that channel.
        </p>
        <FlagList idPrefix="wb" rows={channelRows} />
      </div>
...
/** Small extract to keep WritebackTab readable — same checkbox-row list, 3 places. */
function FlagList({ idPrefix, rows }: {
  idPrefix: string
  rows: Array<{ key: string; label: string; desc: string; checked: boolean; onChange: (v: boolean) => void }>
}) {
  return (
