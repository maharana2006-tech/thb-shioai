-- Was V51 on jagannath_dev, which clashed with V51__bulk_label_jobs_failure_details
-- (Flyway refuses to start with two migrations of one version). IF NOT EXISTS
-- so a database that already ran it under V51 (or where ddl-auto made the
-- table) is not broken by it.
...
CREATE INDEX IF NOT EXISTS idx_dtc_orders_batch_id ON dtc_orders(batch_id);
CREATE INDEX IF NOT EXISTS idx_dtc_orders_tote_number ON dtc_orders(tote_number);
CREATE INDEX IF NOT EXISTS idx_dtc_orders_tenant_id ON dtc_orders(tenant_id);
CREATE INDEX IF NOT EXISTS idx_dtc_orders_order_no ON dtc_orders(order_no);
CREATE INDEX IF NOT EXISTS idx_dtc_orders_created_at ON dtc_orders(created_at);
