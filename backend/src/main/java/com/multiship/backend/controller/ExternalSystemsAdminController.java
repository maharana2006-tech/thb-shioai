...
public class ExternalSystemsAdminController {
    ...
    private final com.multiship.backend.service.externalsystems.writeback.ExternalSystemWritebackDispatcher writebackDispatcher;
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> writebackProbe(
            @PathVariable Long id, @RequestBody(required = false) WritebackProbeRequest req) {
        ExternalSystemConnection c = config.findById(id).orElseThrow(() ->
                new IllegalArgumentException("Connection " + id + " not found."));
        String clientCode = req == null || req.clientCode == null ? "PROBE" : req.clientCode;
    ...
    /** V90 verify — synthetic writeback probe body. All nullable; sensible defaults applied. */
    public static class WritebackProbeRequest {
        public String clientCode;
